...
def setup_new_project(data, folder):
    ...
    try:
        # Check if project type and stack are already selected from a previous run
        if not data.get("project_type"):
            ptype = choose_project_type()
            # ADD THIS CHECK: If user cancelled, exit the setup
            if ptype is None:
                arrow_message("Project setup cancelled.")
                return
        ...
        # Initialize Git only if it hasn't been done before
        if not data.get("git_setup", False):
            progress_message("Initializing Git repository")
            setup_git(folder)
            data["git_setup"] = True
            add_data_to_db(data, str(folder))  # Save this crucial step immediately
        else:
            arrow_message("Git repository already initialized.")

        # Step 3: Add-ons (with improved UI)
        # We check for addons only if they haven't been chosen before.
        if "addons" not in data or not data["addons"]:
            ...
            addons = data["addons"]
            arrow_message(f"Using previously selected add-ons: {', '.join(addons)}")
...
def deploy_app(data):
    ...
    deploy_options.append("Back to Main Menu")

    deploy_choice = Question("Select deployment option:", deploy_options).ask()

    if deploy_choice == "Back to Main Menu":
        return  # This will return to the handle_existing_project loop
    elif deploy_choice == "Deploy with Docker":
